...
public class InsurancePresenterImpl extends BasePresenterImpl<InsuranceView> implements InsurancePresenter {
    ...
    @Inject
    public InsurancePresenterImpl(InsuranceView view, ApiManager apiManager, PreferencesHelper preferencesHelper) {
        super(view);
    ...
    public void getInsurance() {
        ...
        disposable = apiManager.getInsurance(preferencesHelper.getLanguage()).subscribe(s -> {
            try {
                view.showInsurance(Html.fromHtml(new JSONObject(s).getString("text")).toString());
                view.showData();
